...
ServerEvents.commandRegistry(event => {
    ...
    event.register(Commands.literal('splendid_slimes') // The name of the command
        ...
        .then(Commands.literal('clear_current_slimes').requires(source => source.hasPermission(2)) // For debug.
        .then(Commands.argument('target', Arguments.PLAYER.create(event))
        .executes(context => resetCurrentExec(Arguments.PLAYER.getResult(context, 'target'))))
        )
    );
    ...
    const addSlotsExec = (targetPlayer, amount) => {     
     ...
     const resetCurrentExec = (targetPlayer) => {     
        const playerRecord = targetPlayer.server.persistentData.SlimeCountList[targetPlayer.uuid];        
